/* global idahograceBgColors, idahogracePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( idahograceBgColors ).forEach( function( context ) {
					idahograceGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function idahograceGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'idahograce-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#idahograce-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( idahogracePreviewEls[ context ] ) {
				_.each( idahogracePreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		idahograceGenerateColorA11yPreviewStyles( 'content' );
		idahograceGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
